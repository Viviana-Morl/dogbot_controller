...
void DOGCTRL::ctrl_loop() {
    ...
    while( ros::ok() ) {
        ...
        if(now.toSec()>7 && now.toSec()<17)
         {   CoMPosDes << toEigen(des_com_pos)[0]+0.04*sin(toEigen(CoM)[5]),toEigen(des_com_pos)[1]+0.04*sin(toEigen(CoM)[5]), toEigen(des_com_pos)[2], 
        ...
        while((ros::Time::now()-begin).toSec() <   formulation.params_.ee_phase_durations_.at(1)[0] ) {
            ...
            //std::cout<<"Jstcom "<<Jstcom<<std::endl;
            //std::cout<<"Jstj "<<Jstj<<std::endl;
